local library = {
	Version = "0.33",
	WorkspaceName = "Gisper Hub GUI By GustZsy.#0549",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
	...
	colors = {
		main = Color3.fromHex('6600ff'),
		background = Color3.fromRGB(40, 40, 40),
		outerBorder = Color3.fromRGB(15, 15, 15),
		innerBorder = Color3.fromHex('6600ff'),
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Gisper Hub GUI By GustZsy.#0549")
							if not fold then
								makefolder("./Gisper Hub GUI By GustZsy.#0549")
							end
							fold = isfolder("./Gisper Hub GUI By GustZsy.#0549/Themes")
							if not fold then
								makefolder("./Gisper Hub GUI By GustZsy.#0549/Themes")
							end
							fold = isfolder("./Gisper Hub GUI By GustZsy.#0549/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Gisper Hub GUI By GustZsy.#0549 Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Gisper Hub GUI By GustZsy.#0549/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Gisper Hub GUI By GustZsy.#0549/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Gisper Hub GUI By GustZsy.#0549/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Gisper Hub GUI By GustZsy.#0549" then
		library.WorkspaceName = convertfilename(windowName, "Gisper Hub GUI By GustZsy.#0549")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Gisper Hub GUI By GustZsy.#0549") then
						makefolder("./Gisper Hub GUI By GustZsy.#0549")
					end
					local common_string = "./Gisper Hub GUI By GustZsy.#0549/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Gisper Hub GUI By GustZsy.#0549/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Gisper Hub GUI By GustZsy.#0549 Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
